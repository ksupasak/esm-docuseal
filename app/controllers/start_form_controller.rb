...
class StartFormController < ApplicationController
  ...
  def link
    ...
    if params[:data]
      ...
      @template.fields.each do |f|
        ...
        key = f['uuid']
        key = f['name'] if f['name'] and f['name'].size>0
      ...
      puts object.inspect 
       ...
       # @submitter.update(:values=>object)
  ...
  def submitter_params
    params.require(:submitter).permit(:email, :phone, :name, :ref).tap do |attrs|
      attrs[:email] = Submissions.normalize_email(attrs[:email]) if attrs[:email]
      attrs[:email] = attrs[:ref] if attrs[:ref]
      attrs.delete :ref
